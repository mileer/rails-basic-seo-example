module SetSeoTagsHelper
  ...
  def yield_or_default(section, default = '')
    ...
    if tag_text.blank?
      header_text = get_header_text(element: section, page_name: "#{controller_name}.#{action_name}", no_msg: true)
      tag_text = (header_text.present?) ? header_text : default # use optional default as last resort
    end
    tag_text
  end
  ...
  def get_header_text(params)
    ...
    default_key = "header_text.default.#{element}"
  ...
  def branding_text
    t('header_text.branding', default: '')
  end

  def fix_me_statement(thing = '')
    t('header_text.fix_me', thing: thing)
  end
  # /en.header_text.yml
